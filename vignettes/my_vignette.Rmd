...
abstract: >
    When working with omics data you will encounter technical biases. These can often in part be remedied by using appropriate normalization approach. Normalyzer helps this process by both performing a number of popular normalizations as well as a newly implemented approach to counter retention-time dependent bias. Furthermore, NormalyzerDE provides means to easily perform differential expression statistics using either the empirical Bayes Limma approach or ANOVA. Evaluation visualizations are available for both normalization performance measures and as P-value histograms for the subsequent differential expression analysis comparisons. 
...
Installation should preferably be performed using BiocManager:
...
normalyzerDE("vignette_run", 
             "design.tsv", 
             "testout/vignette_run/Loess-G-normalized.txt", 
             outputDir="testoutstat", 
             comparisons=c("1-2", "1-3", "1-4"))
```
