...
outDir <- tempdir()
designFp <- system.file(package="NormalyzerDE", "extdata", "design.tsv")
dataFp <- system.file(package="NormalyzerDE", "extdata", "data.tsv")
...
#designDf <- loadDesign(designFp)
#dataDf <- loadData(dataFp)
...
Alternatively the dataset can be prepared directly as a `SummarizedExperiment` dataset
where the design is assigned as column data, and the annotations as row data.
...
experimentObj <- setupRawContrastObject(bestNormMatPath, designDf, "sample")
