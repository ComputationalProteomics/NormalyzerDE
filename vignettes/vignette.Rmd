...
We recommend installation from Bioconductor with BiocManager:
...
NormalyzerDE: Online Tool for Improved Normalization of Omics Expression Data
and High-Sensitivity Differential Expression Analysis. *Journal of Proteome
Research* **2018**, 10.1021/acs.jproteome.8b00523.
...
The normalization function is expected to take a raw intensity matrix and return
log2-transformed values. We borrow the wrapper function for Loess
normalization from NormalyzerDE. It can be replaced with any custom function
as long as the wrapper has the same input/output format.
...
```{r step1-setup-rawdata}
jobName <- "vignette_stepwise"
...
```{r step4-write-matrices}
jobDir <- setupJobDir(jobName, tempdir())
