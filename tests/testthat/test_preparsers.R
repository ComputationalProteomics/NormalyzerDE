...
test_that("proteiosToNormalyzer reads a Proteios matrix", {
    ...
    utils::write.table(mat, file=fp, sep="\t", row.names=FALSE, col.names=FALSE, quote=FALSE)
...
test_that("maxQuantToNormalyzer parses MaxQuant peptide-level tables", {
    ...
    utils::write.table(in_df, file=fp, sep="\t", row.names=FALSE, quote=FALSE)
    ...
    expect_equal(
        unname(out[1, ]),
        c(
            "Sequence", "Mass", "Proteins", "Leading.razor.protein", "PEP", "Charges",
            "S1", "S2"
        )
    )
...
test_that("maxQuantToNormalyzer parses MaxQuant protein-level tables", {
    ...
    expect_equal(
        unname(out[1, ]),
        c(
            "Protein.IDs", "Majority.protein.IDs", "Fasta.headers",
            "S1", "S2"
        )
    )
})
