...
test_that("normalyzer can read default input files", {
    ...
    designPath <- system.file("extdata", "tiny_design.tsv", package="NormalyzerDE")
...
test_that("normalyzerDE can read default input files", {
    ...
    dataPath <- system.file("extdata", "tiny_data_log2.tsv", package="NormalyzerDE")
    designPath <- system.file("extdata", "tiny_design.tsv", package="NormalyzerDE")
    expect_true(nzchar(dataPath))
    expect_true(nzchar(designPath))
    ...
    expect_true(all(c("4-5_PValue", "4-5_AdjPVal", "4-5_log2FoldChange") %in% colnames(outDf)))
})
