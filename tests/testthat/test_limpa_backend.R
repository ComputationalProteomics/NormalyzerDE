...
test_that("calculateContrasts supports type='limpa'", {
    ...
    if (!requireNamespace("limpa", quietly=TRUE)) {
        expect_error(
            calculateContrasts(nst, comparisons="A-B", condCol="group", type="limpa"),
            "requires.*limpa"
        )
        return()
    }
...
test_that("limpa backend supports one-vs-rest contrasts", {
    ...
    expect_true(all(c("A-rest", "B-rest", "C-rest") %in% names(pairwiseCompsP(out))))
})
...
test_that("limpa backend can summarize peptides to proteins via dpcQuant", {
    ...
    group <- c("A", "A", "A", "B", "B", "B")
    protein_expr <- matrix(rnorm(n_proteins * n_samples, mean=10, sd=1), nrow=n_proteins)
    protein_expr[, group == "B"] <- protein_expr[, group == "B"] + seq_len(n_proteins) / n_proteins

    test_data <- matrix(NA_real_, nrow=length(peptide_protein), ncol=n_samples)
    for (ii in seq_along(peptide_protein)) {
        protein_index <- match(peptide_protein[ii], protein_ids)
        test_data[ii, ] <- protein_expr[protein_index, ] + rnorm(n_samples, sd=0.1)
    }
