...
test_that("Sample names with spaces are preserved", {
  test_data <- matrix(
    c(100, 200, 300, 150, 250, 350, 200, 300, 400, 250, 350, 450),
    nrow = 4,
    byrow = TRUE
  )
  ...
  expect_true(all(
    c("Sample 1", "Sample 2", "Sample 3") %in% colnames(filterrawdata(nds))
  ))
})
...
test_that("Unicode characters in sample names are preserved", {
  ...
  expect_true(all(
    c("Sample α", "Sample β", "Sample γ") %in% colnames(filterrawdata(nds))
  ))
})
...
test_that("setupRawDataObject preserves special character sample names from file", {
  ...
  expect_true(all(
    c("Sample A-1", "Sample A-2", "Sample B.1") %in% data_colnames
  ))
})
...
test_that("Special characters survive normalization + output", {
  ...
  expect_true(all(
    c("Sample A-1", "Sample B.1", "Sample C_1") %in% colnames(output_data)
  ))
})
