...
test_that("generateStatsReport writes a report for multiple contrasts", {
  ...
  pdfPath <- file.path(
    outDir,
    paste0("Norm-stats-report-", safeJobName, ".pdf")
  )
  ...
  expect_gt(
    length(list.files(pngDir, pattern = "\\.png$", full.names = TRUE)),
    0
  )
...
test_that("getSigs supports p-value and fold thresholds", {
  ...
  expect_true(all(lengths(sigs) == nrow(dataMat(nst))))
})
...
test_that("generatePlots writes a Normalyzer report", {
  ...
  nr <- suppressWarnings(normMethods(
    normObj,
  ...
  expect_silent(suppressWarnings(generatePlots(
    tiny_nr,
    outDir2,
    writeAsPngs = FALSE
  )))
