...
generate_output_dir <- function(description, create_dir=TRUE) {
...
do_multiple_runs <- function(run_setting_list, max_cores) {
    ...
    while (current_run <= total_runs) {
        ...
        for (core in 1:max_cores) {
            print(paste("Started run at ", Sys.time()))
...
screen_values <- function(sig_thresholds, do_fdr, rt_windows, window_shifts, 
                          ...
                          quiet_processing=TRUE, subset=FALSE) {
            ...
            for (win_shift in window_shifts) {
                for (low_win_size in lowest_window_size) {
                    for (merge_method in window_merge_method) {
                        ...
                        # descr_string <- paste(sig, fdr, win_shift, low_win_size, merge_method, sep="_")
                        # out_dir <- generate_output_dir(descr_string, create_dir=FALSE)
...
hardcoded_screen_values <- function(do_full_run) {
