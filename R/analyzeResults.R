  ...
  calculateFeatureCVs <- function(feature, groups) {
    featureCVs <- vapply(
      ...
      function(group) {
        ...
        stats::sd(targetFeatures, na.rm = TRUE) /
          mean(targetFeatures, na.rm = TRUE)
      },
      c(1)
    )
    featureCVs
...
calculateAvgReplicateVariation <- function(methodList, sampleReplicateGroups) {
  ...
  calculateReplicateGroupVariance <- function(groupIndices, methodData) {
    ...
    rowVariances <- rowNonNACount *
      matrixStats::rowVars(groupData, na.rm = TRUE)
    replicateGroupVariance <- sum(rowVariances, na.rm = TRUE) /
      sum(rowNonNACount, na.rm = TRUE)
...
calculateCorrSum <- function(
  ...
  for (groupNbr in seq_along(sampleGroupsWithReplicates)) {
    specificReplicateVals <- as.matrix(
      methodData[, which(
        allReplicateGroups == sampleGroupsWithReplicates[groupNbr]
      )]
    )
...
#' condition
#' @keywords internal
calculateANOVAPValues <- function(
  methodList,
...
findLowlyVariableFeaturesCVs <- function(referenceFDR, methodList) {
  ...
  pThresLowVal <- min(utils::head(
    rev(sort(referenceFDRWoNA)),
    n = fivePercCount
  ))
  ...
  calculateAverageCVs <- function(methodData, lowVarContrast) {
    ...
    averageCVs <- mean(
      matrixStats::rowSds(lowVarFeatures, na.rm = TRUE) /
        rowMeans(lowVarFeatures, na.rm = TRUE) *
        100,
      na.rm = TRUE
    )
    averageCVs
