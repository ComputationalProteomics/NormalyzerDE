...
getRTNormalizedMatrix <- function(
  rawMatrix,
  retentionTimes,
  normMethod,
  stepSizeMinutes = 1,
  ...
  for (windowStart in seq(startVal, endVal, stepSizeMinutes)) {
    ...
    if (noLogTransform) {
      processedNormalizationRows <- normMethod(
        normalizationRows,
...
#' @keywords internal
getWidenedRTRange <- function(
  rtStart,
  rtEnd,
  minimumDatapoints,
  retentionTimes,
  sortedRetentionTimes = NULL,
  ...
  stopifnot(
    totalBefore + totalAfter + length(currentRTSlice) == length(retentionTimes)
  )
