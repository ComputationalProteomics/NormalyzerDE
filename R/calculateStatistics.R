...
reduceTechnicalReplicates <- function(se, techRepColName, sampleColName) {
  ...
  indices <- lapply(unique(techRepGroups), function(i) {
    which(techRepGroups %in% i)
  })

  # Reduce design
  collDesignMatList <- lapply(indices, function(inds) {
    designDf[inds[1], ]
  })
  ...
  collapsedNames <- unlist(lapply(indices, function(inds) {
    paste(designDf[[sampleColName]][inds], collapse = ".")
  }))
  ...
  collDataMat <- as.matrix(data.frame(lapply(
    indices,
    function(inds) {
      rowMeans(dataMat[, inds, drop = FALSE], na.rm = TRUE)
    }
  )))
...
generateStatsReport <- function(
  ...
  writePage(
    ...
    type = "MA",
    sigThres = sigThres,
    sigThresType = sigThresType,
    log2FoldThres = log2FoldThres
  )
...
plotComparisonVenns <- function(
  ...
  for (indOut in seq_len(compCount - 1)) {
    for (indIn in (indOut + 1):compCount) {
      ...
      contraCount <- length(which(
        sign(folds[[indIn]][bothSigInds]) != sign(folds[[indOut]][bothSigInds])
      ))
      ...
      plt <- ggplot2::ggplot(data = df.venn) +
        ...
        ggplot2::scale_fill_manual(
          values = c('cornflowerblue', 'firebrick', 'gold')
        ) +
  ...
  for (i in seq_along(contrastPLists)) {
    ...
    if (type == "Volcano") {
      plots[[i]] <- ggplot2::ggplot(
        df,
        ggplot2::aes(fold, -log10(pVals), color = sig)
      ) +
      ...
      plots[[i]] <- ggplot2::ggplot(
        df,
        ggplot2::aes(expression, fold, color = sig)
      ) +
        ggplot2::geom_point(size = 1, alpha = 0.5, na.rm = TRUE) +
...
plotContrastPCA <- function(
  ...
  if (max(pcs) > ncol(dfOut)) {
    ...
    grid::pushViewport(grid::viewport(layout = currentLayout))
    printPlots(
      list(messagePlot(paste0(
        "PCA unavailable (only ",
        ncol(dfOut),
        " PC(s))."
      ))),
      title,
  ...
  percentageVar <- paste0(
    colnames(dfOut),
    " (",
    paste0(as.character(percentageVar), "%)")
  )
  ...
  splitContrast <- function(contrast, sep) {
    ...
    c(
      paste(parts[seq_len(length(parts) - 1)], collapse = sep),
      parts[length(parts)]
    )
  }
  ...
  for (contrast in contrasts) {
    ...
    if (contrastLevels[1] %in% groups && contrastLevels[2] %in% groups) {
      ...
      dfOut$group <- activeLevelsFactor(
        groups,
        list(
          high = contrastLevels[1],
          low = setdiff(otherGroups, contrastLevels[1])
        )
      )
    } else {
      dfOut$group <- "other"
    ...
    plt <- ggplot2::ggplot(
      ...
      ggplot2::scale_color_manual(
        values = c(high = "#00AAAA", low = "#AA0000", other = "#BBBBBB")
      ) +
