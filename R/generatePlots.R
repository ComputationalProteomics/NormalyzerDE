...
generatePlots <- function(
  ...
  writePage(
    ...
    jobdir,
    writeAsPngs,
    sprintf("%s/%s_%s", pngDir, nextPageNo(iter = FALSE), "front_page.png"),
    currentjob,
    currentFont
  )
  isLimitedRun <- singleReplicateRun(nds) || isTinyRun(nds)
  writePage(
    plotSampleMappingPage,
    jobdir,
    writeAsPngs,
    sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "sample_setup.png"),
    nr,
    currentFont,
    currentLayout,
    currentjob,
    nextPageNo(iter = FALSE)
  )
  writePage(
  ...
  if (!isLimitedRun) {
    ...
    writePage(
      ...
      sprintf(
        ...
        pngDir,
        nextPageNo(iter = TRUE),
        "replicate_variation_rel_to_log2.png"
      ),
      nr,
      currentLayout,
      nextPageNo(iter = FALSE)
    ...
    writePage(
      ...
      jobdir,
      writeAsPngs,
      sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "ma.png"),
      nr,
      currentLayout,
      nextPageNo(iter = FALSE)
    )
  ...
  writePage(
    ...
    jobdir,
    writeAsPngs,
    sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "scatter.png"),
    nr,
    currentLayout,
    nextPageNo(iter = FALSE)
  )
  writePage(
    ...
    jobdir,
    writeAsPngs,
    sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "qqplots.png"),
    nr,
    currentLayout,
    nextPageNo(iter = FALSE)
  )
  writePage(
    ...
    jobdir,
    writeAsPngs,
    sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "boxplot.png"),
    nr,
    currentLayout,
    nextPageNo(iter = FALSE)
  )
  writePage(
    ...
    jobdir,
    writeAsPngs,
    sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "rel_log_expr.png"),
    nr,
    currentLayout,
    nextPageNo(iter = FALSE)
  )
  writePage(
    ...
    jobdir,
    writeAsPngs,
    sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "density.png"),
    nr,
    currentLayout,
    nextPageNo(iter = FALSE)
  )
  writePage(
    ...
    jobdir,
    writeAsPngs,
    sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "mds.png"),
    nr,
    currentLayout,
    nextPageNo(iter = FALSE)
  )
  ...
  if (!isLimitedRun) {
    writePage(
      ...
      jobdir,
      writeAsPngs,
      sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "mean_sd.png"),
      nr,
      currentLayout,
      nextPageNo(iter = FALSE)
    )
    writePage(
      ...
      jobdir,
      writeAsPngs,
      sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "correlation.png"),
      nr,
      currentLayout,
      nextPageNo(iter = FALSE)
    )
  ...
  writePage(
    ...
    jobdir,
    writeAsPngs,
    sprintf("%s/%s_%s", pngDir, nextPageNo(iter = TRUE), "dendrogram.png"),
    nr,
    currentLayout,
    nextPageNo(iter = FALSE)
  )
...
#' @return None
#' @keywords internal
plotSampleMappingPage <- function(
  nr,
  currentFont,
  currentLayout,
  currentjob,
  ...
  groupList <- lapply(
    ...
    function(lv, groups, groupCounts) {
      list(
        groupNbr = lv,
        groupString = groups[[lv]],
        groupCount = groupCounts[[lv]]
      )
    },
    groups = groupLevels,
    groupCounts = condCounts
  )
  ...
  lapply(
    seq_len(length(groupStringsWHead)),
    function(i, stringsToShow) {
      yPos <- 0.9 - i * 0.02
      xDelta <- 0.15
      grid::grid.text(
        stringsToShow[[i]]$groupNbr,
        x = 0.01,
        y = yPos,
        hjust = 0
      )
      grid::grid.text(
        stringsToShow[[i]]$groupString,
        x = 0.01 + xDelta,
        y = yPos,
        hjust = 0
      )
      grid::grid.text(
        stringsToShow[[i]]$groupCount,
        x = 0.01 + 2 * xDelta,
        y = yPos,
        hjust = 0
      )
...
plotReplicateVarAndStableVariables <- function(nr, currentLayout, pageno) {
  ...
  graphics::text(
    abc,
    avgcvmempdiff,
    labels = round(avgcvmempdiff, digits = 0),
    pos = 3,
    las = 2
  )
...
plotMA <- function(nr, currentLayout, pageno) {
  ...
  Malist <- lapply(seq_along(normalizedDataList), function(i) {
    ...
    function() {
      ...
      firstColWoNA <- methodDataFirstCond[
        !is.na(methodDataFirstCond[, 1]),
        ,
        drop = FALSE
      ]
...
plotRLE <- function(nr, currentLayout, pageno) {
  ...
  for (i in seq_along(methodlist)) {
    deviations <- methodlist[[i]] -
      matrixStats::rowMedians(methodlist[[i]], na.rm = TRUE)
    graphics::boxplot(
      deviations,
      outcol = "lightgray",
      cex = 0.1,
...
plotDensity <- function(nr, currentLayout, pageno) {
  ...
  for (i in seq_along(methodlist)) {
    ...
    graphics::plot(
      ...
      xlab = "",
      ylab = "",
      ylim = c(min(tempd$y), max(tempd$y) * 1.5),
      main = methodnames[i],
      lty = 2,
      lwd = 1,
      col = "darkgray"
    ...
    for (j in 2:ncol(methodData)) {
      graphics::lines(
        stats::density(methodData[, j], na.rm = TRUE),
        ,
        lty = 2,
        lwd = 1,
        col = "darkgray"
...
plotMDS <- function(nr, currentLayout, pageno) {
  ...
  printMeta(
    paste(
      "MDS plots - Built from",
      ncol(d),
      "variables with non-missing data",
      sep = " "
    ),
    pageno,
    currentjob,
...
plotCorrelation <- function(nr, currentLayout, pageno) {
  ...
  abc <- graphics::boxplot(
    perdf,
    main = "Pearson correlation - Intragroup",
    names = c(methodnames),
    border = "red",
    density = 20,
    cex = 0.3,
  ...
  abc <- graphics::boxplot(
    spedf,
    main = "Spearman correlation - Intragroup",
    names = c(methodnames),
    border = "red",
    density = 20,
    cex = 0.3,
...
plotDendrograms <- function(nr, currentLayout, pageno) {
  ...
  printMeta(
    paste(
      "Dendrograms - Built from",
      ncol(scaledTransposedMatrix),
      "variables containing non-missing data",
      sep = " "
    ),
    pageno,
    currentjob,
