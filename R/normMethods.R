...
normMethods<-function(datafile,currentjob)
{
    ...
    #NORMALIZATION within REPLICATES RLR, VSN and  Loess
  {
    ...
    if(HKflag==T){
      methodlist<-list(data2log2,data2limloess,fittedLR,data2vsnrep,data2loess,globalfittedRLR, data2vsn,data2GI,data2med,data2mean,data2ctrlog,data2quantile)
  ...
  {
    write.table(file=paste(jobdir,"/",methodnames[i],"-normalized.txt",sep=""),cbind(getrawdata[-(1:2),(1:(length(getEDdata)-length(filterED)))],methodlist[[i]]),sep="\t",row.names=F,col.names=getrawdata[2,],quote=F)
  }
  if(HKflag==T)
  {
    write.table(file=paste(jobdir,"/housekeeping-variables.txt",sep=""),Hkvar,sep="\t",row.names=F,col.names=getrawdata[2,],quote=F)
  }
  write.table(file=paste(jobdir,"/submitted_rawdata.txt",sep=""),cbind(getrawdata[-(1:2),(1:(length(getEDdata)-length(filterED)))],filterrawdata),sep="\t",row.names=F,col.names=getrawdata[2,],quote=F)
